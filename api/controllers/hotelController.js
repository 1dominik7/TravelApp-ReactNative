...
module.exports = {
    ...
    getHotelsByCountry: async (req,res,next) =>{
        ...
        try {
            const hotels = await Hotel.find({country_id: countryId}, '_id review rating imageUrl title country_id location price')
            .limit(limit)
    ...
    getHotelById: async (req, res, next) => {
        ...
        try {
            ...
            if(!hotel){
                return res.status(404).json({status:false, message:"Hotel doesn't exist"})
            }
